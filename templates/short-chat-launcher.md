...
The short launcher must be written to disk. When the target runs in the same runtime and direct dispatch is available, use the saved launcher as the dispatch seed and record `dispatchChannel: agent_thread_spawn` or `dispatchChannel: one_click`. When the target runs in another runtime, record `dispatchChannel: runtime_bridge` and queue the bridge event in `.openaccp/coordination/bridge-events.jsonl`. When direct or bridge dispatch is unavailable, print the short launcher in chat as a fenced `prompt` block and record `dispatchChannel: manual_paste`. A saved `.short.md` file is required for audit, but a file link, attachment, file list, or `Get-Content` command is not a substitute for the manual fallback chat block.
