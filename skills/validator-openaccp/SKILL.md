...
Use `frontier-contract` before launching or reusing a Frontier prompt. It checks the B2 lane contract, `openaccp-frontier-orchestration-contract.v1` JSON block, subagent-first dispatch, runtime-aware child ledger fields, return event protocol, branch return gate, worktree decision, human-readable reporting, and fallback-only child launcher rule.

Use `runtime-boundary` during Primary startup before B2 Frontier dispatch. It records Primary runtime identity, notification bridge policy, bridge event queue, working directory, product repo status/path, Primary-inferred base branch, source roots, test entrypoints, worktree policy, inference evidence, ambiguity notes, writable/read-only/forbidden paths, side-effect level, data risk, unresolved owner inputs, and `b2DispatchGate`.

Use `lane-registry` for the active Primary/Frontier control plane. It records project complexity, Primary runtime, Frontier dispatch mode, dispatch channel, lane-count reason, lane ids, objectives, lane runtime, same-runtime or cross-runtime relation, notification bridge policy, authority, assigned CARDs, runtime boundary, child ledger, latest consume refs, return gate status, and per-lane `b2DispatchGate`. Cross-runtime Frontier lanes require a bridge policy, must use `runtime_bridge` or `manual_paste`, and must queue parent wakeups while the parent is busy.

Use `child-ledger` inside each Frontier lane. It records every worker/reviewer/discovery/validation child with Prompt ID, taskId, authority, effects, parentRuntime, childRuntime, runtimeRelation, returnEventStatus, wakeStatus, parentConsumeDuePolicy, dispatch status, handoff status, consume status, and remaining risk. Response ID and handoffId become required by lifecycle when a child returns or a handoff is present. A returned present handoff with `consumeStatus: not_consumed` must be visible as `parent_consume_pending` or `parent_consuming`; cross-runtime returns require `notificationBridgeRef` plus a queued/requested/delivered/unavailable/failed wake state. B3 child authority is invalid.
...
Use `launcher-output` when a response starts or returns Primary or Frontier launchers. It reads `dispatchChannel`. `agent_thread_spawn` and `one_click` are same-runtime channels and may pass without human paste instructions only when the response includes an explicit dispatch channel line plus a successful result line such as `Spawn result: dispatched` or `Launch result: started`. `runtime_bridge` is the cross-runtime channel and requires `Runtime relation: cross_runtime`, a successful `Bridge result`, and a `Bridge event` locator. `manual_paste` must include copyable fenced `prompt` blocks and left-sidebar paste guidance; file links and `Get-Content` commands are not substitutes.

Use `bridge-event` for one queued or delivered cross-runtime event. `parent_to_child` starts or wakes a Frontier. `child_to_parent` wakes Primary for consume. The ruleset checks direction, event type, runtime relation, expected action, and safe busy policy.
