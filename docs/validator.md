...
- `launcher-output`: response-log or chat-output text that declares a dispatch channel. `agent_thread_spawn` and `one_click` outputs may skip copy/paste instructions only when they include an explicit dispatch channel line plus a successful result line such as `Spawn result: dispatched` or `Launch result: started`. `manual_paste` outputs must include copyable fenced `prompt` blocks plus natural-language instruction to open a new left-sidebar thread and paste the launcher. Manual fallback still rejects file-link-only and `Get-Content` substitutes.
