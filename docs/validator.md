...
- `launcher-output`: response-log or chat-output text that declares a dispatch channel. `agent_thread_spawn` and `one_click` outputs are same-runtime channels and may skip copy/paste instructions only when they include an explicit dispatch channel line plus a successful result line such as `Spawn result: dispatched` or `Launch result: started`. `runtime_bridge` is the cross-runtime channel and requires `Runtime relation: cross_runtime`, a successful `Bridge result`, and a `Bridge event` locator. `manual_paste` outputs must include copyable fenced `prompt` blocks plus natural-language instruction to open a new left-sidebar thread and paste the launcher. Manual fallback still rejects file-link-only and `Get-Content` substitutes.
...
- `runtime-boundary`: Primary startup boundary with Primary runtime identity, notification bridge policy, bridge event queue, working directory, product repo status/path, inferred base branch, source roots, test entrypoints, worktree policy, inference evidence, ambiguity notes, writable/read-only/forbidden paths, side effects, data risk, unresolved owner inputs, and `b2DispatchGate`.
...
- `lane-registry`: Primary/Frontier lane registry with project complexity, Primary runtime, Frontier dispatch mode, dispatch channel, lane-count reason, lane runtime, same-runtime or cross-runtime relation, notification bridge policy, assigned CARDs, authority, runtime boundary ref, child ledger ref, consume refs, return-gate status, and per-lane `b2DispatchGate`. Cross-runtime Frontier lanes reject native `agent_thread_spawn` and `one_click`; use `runtime_bridge` or `manual_paste`.
- `bridge-event`: one cross-runtime queue event. `parent_to_child` events request Frontier startup; `child_to_parent` events request parent consume. The ruleset checks runtime relation, event direction, expected action, and busy policy.
- `child-ledger`: Frontier child lifecycle registry with promptId, taskId, authority, effects, runtime relation, return event status, wake status, dispatch status, handoff status, consume status, and remaining risk. Response ID, handoffId, and parent consume refs are lifecycle fields.
