...
- `execution-boundary`: Primary startup boundary with working directory, product repo status/path, inferred base branch, source roots, test entrypoints, worktree policy, inference evidence, ambiguity notes, writable/read-only/forbidden paths, side effects, data risk, unresolved owner inputs, and `b2DispatchGate`.
...
```bash
python tools/openaccp_validate.py --artifact .openaccp/coordination/execution-boundary.json --ruleset execution-boundary --strict
