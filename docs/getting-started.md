...
Use the Bootstrap path when the source pack or task card is missing. Use the Coordination path when the project needs the larger multi-lane package: scope boundary, assumptions ledger, CARD registry, runtime boundary, current manifest, sequence registry, lane registry, child ledger, source status registry, decision registry, and Frontier closure. In multi-runtime setups, Primary records each Frontier runtime and uses `.openaccp/coordination/bridge-events.jsonl` with `openaccp notify-dispatch` and `openaccp notify-return` so dispatch and returned evidence stay visible until the correct orchestrator consumes them.
